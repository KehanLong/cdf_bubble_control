...
class RobotCDFDataset:
    def __init__(self, configurations, points, cdf_values):
        self.configurations = torch.FloatTensor(configurations)
        self.points = torch.FloatTensor(points)
        self.cdf_values = torch.FloatTensor(cdf_values)  # Now shape (N, num_links)
        
        # Compute sin and cos of angles
        self.sin_configs = torch.sin(self.configurations)
        self.cos_configs = torch.cos(self.configurations)

    def __getitem__(self, idx):
        config = self.configurations[idx]
        sin_config = self.sin_configs[idx]
        cos_config = self.cos_configs[idx]
        point = self.points[idx]
        cdf_value = self.cdf_values[idx]  # CDF

        # Concatenate features
        features = torch.cat([config, sin_config, cos_config, point])
        return features, cdf_value
