...
def main():
    ...
    # Switch between 2-link and 4-link cases
    two_link_mode = True  # Set to False for 4-link case
    
    if two_link_mode:
        link_lengths = [2., 2.]  # 2-link robot
        joint_pairs = [(0, 1)]  # Only one possible joint pair
        fig_sdf, axs_sdf = plt.subplots(1, 1, figsize=(10, 10))
        fig_cdf, axs_cdf = plt.subplots(1, 1, figsize=(10, 10))
        axs_sdf, axs_cdf = [axs_sdf], [axs_cdf]  # Wrap in list for consistent indexing
    else:
    ...
    obstacle = [-0.1, 0.55, 0.]  # [x, y, radius]
    ...
    mode_str = "2link" if two_link_mode else "4link"
    fig_sdf.savefig(f'{mode_str}_sdf_comparison.png', dpi=300, bbox_inches='tight')
    fig_cdf.savefig(f'{mode_str}_cdf_comparison.png', dpi=300, bbox_inches='tight')

    print(f"Analysis complete. Check '{mode_str}_sdf_comparison.png' and '{mode_str}_cdf_comparison.png' for results.")
